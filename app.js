  ...
  return {
    ...
    courses: {
      [courseId]: {
        ...
        students: {
          [s1]: { id:s1, name:'Ana Gómez', condition:'cursa', stats:{present:2, absent:1, later:0}, history:[{id:uid('h'), date: todayStr(), status:'present'}], grades:[] },
          [s2]: { id:s2, name:'Bruno Díaz', condition:'cursa', stats:{present:1, absent:1, later:1}, history:[], grades:[] },
...
function RollCallCard({ students, onMark, onUndo, selectedDate }){
  ...
  React.useEffect(()=>{ setOrder(students.map(s=>s.id)); setIndex(0); setOps([]); }, [students.map(s=>s.id).join('|')]);
...
function StudentsTable({ course, students, onAdd, onEdit, onDelete, onShowAbsences }){
  ...
  const sorted = React.useMemo(()=>Object.values(students).sort((a,b)=>a.name.localeCompare(b.name)),[students]);
  return e('div', { className:'p-4 md:p-6' },
...
function AbsencesModal({ open, student, onClose, onApplyChange }){
  ...
  React.useEffect(()=>{ setChoices({}); }, [open, student && student.id]);
...
function App(){
  ...
  const [absencesOpen, setAbsencesOpen] = React.useState(false);
  const [absencesStudentId, setAbsencesStudentId] = React.useState(null);

  React.useEffect(()=>{ saveState(state); }, [state]);
  React.useEffect(()=>{ window.__openExport = () => setExportOpen(true); return () => { try{ delete window.__openExport; }catch(_){}}; }, []);

  const selectedCourse = state.courses[state.selectedCourseId];
  const studentsArr = React.useMemo(()=>Object.values(selectedCourse.students).sort((a,b)=>a.name.localeCompare(b.name)), [selectedCourse]);
  ...
  return e('div', null,
    e(Header, { selectedDate: state.selectedDate, onChangeDate: (v)=>setSelectedDate(v) }),
